...
export default function Dashboard() {
  ...
  async function handleAddWeekend(e: React.FormEvent) {
    ...
    } else if (newStatus !== "scheduled" && newAskTeamId) {
      // Open/busy day, but you still want to ping a specific team about it —
      // send the same confirmation-style request without locking in your own
      // schedule yet. Your board updates once they accept.
      const { error: reqError } = await supabase.from("game_requests").insert({
        from_team_id: team.id,
        to_team_id: newAskTeamId,
        source_weekend_id: inserted.id,
        kind: "confirmation",
        from_wants_to_host: newIsHome,
      });
  ...
  async function handleApproveJoin(req: TeamJoinRequest) {
    ...
    await supabase
      ...
      .update({
        ...
        member_type: req.requested_member_type,
      })
  ...
  function handleExportExcel() {
    ...
    function opponentLabelFor(w: Weekend) {
    ...
    const rows = weekends
      ...
      .map((w) => ({
        Date: w.date,
        Status: w.status === "scheduled" ? "Scheduled" : w.status === "open" ? "Open" : "Busy",
        Opponent: w.status === "scheduled" ? opponentLabelFor(w) : "",
        "Home / Away": w.status === "scheduled" ? (w.is_home ? "Home" : "Away") : w.preference ?? "",
        Time: w.game_time ?? "",
  ...
  return (
    <div className="space-y-10">
      ...
      {profile?.member_type === "staff" && pendingCount > 0 && (
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-display text-lg font-semibold">Your games</h2>
          <button
            onClick={handleExportExcel}
            className="text-xs font-medium text-ice-dim hover:text-ice border border-line-white rounded-md px-3 py-1.5 flex items-center gap-1.5"
          >
          ...
          <div className="space-y-2">
            {scheduledGames.map((w) => (
              <GameRow
                ...
                schools={schools}
                myTeamId={team.id}
                isStaff={profile?.member_type === "staff"}
              />
            ))}
          </div>
        )}
        ...
        {weekends.filter((w) => w.status !== "scheduled").length === 0 ? (
          <p className="text-sm text-ice-dim mb-4">
            {profile?.member_type === "staff"
              ? "Nothing marked yet — add a weekend below."
              : "Nothing marked yet."}
          </p>
        ) : (
          <div className="mb-4">
            <ScheduleGrid
              ...
              onDelete={profile?.member_type === "staff" ? handleDeleteWeekend : undefined}
            />
          </div>
        )}
          ...
          {(newStatus === "open" || newStatus === "busy") && (
            <div className="flex flex-wrap items-end gap-3 pt-1 border-t border-line-white/50">
              <p className="w-full text-xs text-ice-dim pt-3">
                Optional — ping a specific team about this day. They&apos;ll get a request to
                confirm; nothing changes on your board until they accept.
              </p>
              <div>
                <label className="block text-xs text-ice-dim mb-1">Ask a team</label>
                <select
                  value={newAskTeamId}
                  onChange={(e) => setNewAskTeamId(e.target.value)}
                  className="bg-rink border border-line-white rounded-md px-3 py-2 text-sm outline-none focus:border-faceoff-blue"
                >
                  <option value="">Don&apos;t ask anyone</option>
                  {allTeams
              ...
              {newAskTeamId && (
                <div>
                  <label className="block text-xs text-ice-dim mb-1">If they say yes, you&apos;d</label>
                  <select
          ...
          {newStatus === "scheduled" && (
            <div className="flex flex-wrap items-end gap-3 pt-1 border-t border-line-white/50">
              ...
              {newOpponentTeamId === "other" && (
                <div>
                  <label className="block text-xs text-ice-dim mb-1">Opponent school</label>
                  <select

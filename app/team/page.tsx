...
export default function TeamProfilePage() {
  ...
  async function handleDeleteTeam() {
    ...
    const confirmText = prompt(
      `This permanently deletes ${team.short_name} — its schedule, messages, and roster for everyone. Type the team's short name (${team.short_name}) to confirm.`
    );
    if (confirmText !== team.short_name) {
      if (confirmText !== null) alert("Name didn't match — nothing was deleted.");
      return;
    }
  ...
  return (
    <div className="space-y-10">
              ...
              <div>
                <p className="text-sm font-medium">
                  ...
                  {member.member_type === "player" && (
                    <span className="ml-1.5 text-[10px] font-mono uppercase tracking-wide text-ice-dim/70 border border-line-white rounded px-1.5 py-0.5">
                      Player
                    </span>
                  )}
                </p>
      ...
      <section>
        ...
        {profile.is_team_admin && (
          <div className="rounded-lg border border-board-red/40 bg-board-red/5 p-4 flex items-center justify-between gap-3 flex-wrap mt-3">
            <p className="text-sm text-ice-dim">
              Permanently deletes {team.short_name} — schedule, messages, requests, and roster
              for everyone. This cannot be undone.
            </p>
            <button
              onClick={handleDeleteTeam}
              disabled={deletingTeam}
              className="px-4 py-2 text-sm font-medium rounded-md bg-board-red text-ice hover:bg-board-red/90 shrink-0 disabled:opacity-50"
            >
              {deletingTeam ? "Deleting..." : `Delete ${team.short_name}`}
            </button>
          </div>
        )}
      </section>
